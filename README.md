...
- OpenClaw程序自动查找与启动（减少“找不到文件”问题）
- 连续语音（更接近电话式持续对话）
- 3D模型上传与浏览器预览（GLB/GLTF/OBJ/FBX）
...
### E. 提示“找不到 OpenClaw 文件”

如果你一直连不上真实 OpenClaw，可以先点界面里的 `启动模拟OpenClaw`。
这样你可以先完整测试语音对话、打断、3D预览等功能，不会被连接问题卡住。
...
建议先点“自动查找”，找到后再点“启动OpenClaw”，随后点“自动检测”连接服务。

### F. 3D真人形象怎么用

1. 先点击“应用参考形象”（会套用你给的霓虹潮流风角色设定）
2. 点击“上传3D模型”（建议 `.glb`）
3. 点击“打开3D预览”
4. 程序会在浏览器里以 3D 方式预览模型（可旋转/缩放）

> 说明：Tkinter 本身不适合高质量实时 3D 渲染，因此这里采用浏览器 `model-viewer` 做 3D 展示。


### G. `.openclaw` 工作目录约定

按你的要求，程序现在默认把 `~/.openclaw` 作为 OpenClaw 工作目录：
- 会优先在 `~/.openclaw` / `~/.openclaw/bin` 查找 OpenClaw 可执行文件
- 点击“启动OpenClaw”时会优先以该目录作为启动工作目录

只要你的 OpenClaw 放在这个目录结构中，通常可以直接自动查找并启动。
